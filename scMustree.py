...
class MusTree:
	...
	def __init__(self, data_matrix: np.ndarray, dataName: str = None, cellNames: list = None, geneNames: list = None, seed: int = None, save_path: str = None):
		...
		self.node_anns = None
		self.node_to_cells_anns =  np.array(['unknown'] * n_cells, dtype=object)
	...
	def edit_node_annotation(self, node_list, annotations):
		if len(node_list) != len(annotations):
			raise ValueError("The lengths of node_list and annotations must be the same.")
		if not all(node in self.G for node in node_list):
			raise ValueError("One or more nodes in node_list are not present in the graph.")
		if self.G is None:
			raise ValueError("Please infer tree dirst!")
		for i, node in enumerate(node_list):
			self.node_anns[list(self.G.nodes()).index(node)] = str(annotations[i])
		
		
	def edit_cell_annotation_within_Nodes(self, node_list, annotations):
		if len(node_list) != len(annotations):
			raise ValueError("The lengths of node_list and annotations must be the same.")
		if not all(node in self.G for node in node_list):
			raise ValueError("One or more nodes in node_list are not present in the graph.")
		if len(self.nodes_to_cellids) == 0:
			raise ValueError("Please infer tree dirst!")
		for i, node in enumerate(node_list):
			idx = self.nodes_to_cellids[node]
			self.node_to_cells_anns[idx] = str(annotations[i])
	...
	def infer_multiscale_tree(self):
		...
		self.node_anns = ['unknown'] * len(list(self.G.nodes))
	...
	def plot_tree(self, path=None, label=None, remain_types=None,  plot_name=None, 
		...
		if label is not None:
			assert len(label) == self.n_cells or len(label) == len(list(self.G.nodes)), \
			f"Number of labels ({len(label)}) must match either the number of cells ({self.n_cells}) or the number of nodes ({len(self.G.nodes)})"
		...
		if not isinstance(plot_name, str):
			raise TypeError(f"plot_name must be a string, but got {type(plot_name).__name__}. Please provide a custom string identifier for the plot.")
		...
		if label is not None:
			if len(label) == self.n_cells:
			
				types = list(set(label))
				if 'unknown' in types:
					remain_types=list(set(types)-set(['unknown']))
				...
				if plot_name is None:
					...
					plt.savefig(fname=path+plot_name+'.png', bbox_inches='tight',  dpi = 2000, pad_inches=0.05)
				plt.show()
				...
				if remain_types is None:
					remain_types = types
					fn = "dendrogram_with_nodes_labels"
				else:
					fn = "dendrogram_with_nodes_"+'_'.join(remain_types)+"_labels"
				remain_types = list(set(remain_types))
				...
				for i in list(self.G.nodes):
					if label[i] == 'unknown':
						...
						self.G.nodes[i]['group'] = typetoid[label[i]]
						node_colors.append(color_list[typetoid[label[i]]])
	...
	def plot_gene_expression_on_tree(self, path=None, markers_list=None, markers_name=None,  plot_name=None, 
		...
		if plot_name is None:
			...
			plt.savefig(fname=path+plot_name+'.png', dpi = 2000, bbox_inches = 'tight')
		plt.show()
		
	
	def map_tree_nodes_to_UMAP(self, path=None, nodes_list=None, label=None, umap_embedding=None, plot_name=None, 
		...
		if nodes_list is not None:
			...
			plt.subplots_adjust(right=0.8)
			if plot_name is None:
				plt.savefig(fname=path+self.dataName+'_UMAP_with_nodesid_'+'_'.join([str(i) for i in nodes_list])+'.png', bbox_inches='tight', dpi = 2000, pad_inches=0.05)
			else:
				plt.savefig(fname=path+plot_name+'.png', bbox_inches='tight', dpi = 2000, pad_inches=0.05)
			plt.show()		
		else:
			...
			plt.subplots_adjust(right=0.8)
			if plot_name is None:
				plt.savefig(fname=path+self.dataName+'_UMAP_with_label.png', bbox_inches='tight', dpi = 2000, pad_inches=0.05)
			else:
				plt.savefig(fname=path+plot_name+'.png', bbox_inches='tight', dpi = 2000, pad_inches=0.05)
			plt.show()
